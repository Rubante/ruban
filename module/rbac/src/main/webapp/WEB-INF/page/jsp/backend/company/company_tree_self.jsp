...
(function(){
	...
	function zTreeBeforeClick(treeId, treeNode, clickFlag) {
		...
		if(treeNode.parentTId){
			...
			if (treeNode.isParent) {
				// 查询条件
				${param.module}.companyList(treeNode.id);
			} else {
				${param.module}.companyList();
			}
			...
			${param.module}.rootCompanyList(treeNode.sId);
			isRoot = true;
		}
	};
	...
	function zTreeBeforeDblClick(treeId, treeNode, clickFlag){
		...
		if (!treeNode.isParent && treeNode.parentTId) {
			${param.module}.detail(treeNode.id);
		}
	};
	...
	companyTree.selectCurrent = function(treeId){
		...
		if(currentId != "" && !isRoot) {
			// 查询条件
			${param.module}.companyList(currentId);
		} else {
			var nodes = treeObj.getNodes();
			
			${param.module}.rootCompanyList(nodes[0].sId);
			currentId = nodes[0].sId;
		}
