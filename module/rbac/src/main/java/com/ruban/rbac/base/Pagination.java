...
public class Pagination<T> {
    ...
    public void gereate(ResultInfo<T> resultInfo) {

        list = new ArrayList<>();
        int current = resultInfo.getPageNum();

        int beforeCount = 0;
        // 当前页码前面
        for (int i = current; current <= 5 && i > 0
                || (resultInfo.getPages() - current) < 5 && (i > resultInfo.getPages() - 10) && i > 0
                || i > current - 5 && i > 0; i--) {
        ...
        // 当前页码后面
        int i = current + 1;
        int maxPage = resultInfo.getPages() < 10 ? resultInfo.getPages() : 10;
        while (beforeCount < resultInfo.getPages() && i <= (current + maxPage - beforeCount)) {
            Pagination<T> pagination = new Pagination<>();
            pagination.setValue(String.valueOf(i));
            list.add(0, pagination);
            i++;
